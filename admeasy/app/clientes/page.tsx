...
const tipoBadge: Record<string, string> = {
  locatario: 'badge badge-blue',
  locador: 'badge badge-green',
  comprador: 'badge badge-purple',
  vendedor: 'badge badge-gray',
...
export default function ClientesPage() {
  ...
  async function salvarCliente(e: React.FormEvent) {
    ...
    if (!userData.user) { setErro('Sessão expirada.'); setSalvando(false); return }
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        {showForm && (
          <div className="card mb-6">
            <h2 className="text-sm font-semibold mb-4">Cadastrar novo cliente</h2>
            <form onSubmit={salvarCliente}>
              <div className="grid grid-cols-2 gap-3">
                <div className="col-span-2">
                  <label className="label">Nome completo *</label>
                  <input className="input" required value={form.nome} onChange={e => setForm({...form, nome: e.target.value})} placeholder="Nome completo" />
                </div>
                <div>
                ...
                <div>
                  ...
                  <input className="input" value={form.profissao} onChange={e => setForm({...form, profissao: e.target.value})} placeholder="Ex: Engenheiro" />
                </div>
                <div>
                  <label className="label">Renda mensal (R$)</label>
                  <input className="input" type="number" value={form.renda_mensal} onChange={e => setForm({...form, renda_mensal: e.target.value})} placeholder="0,00" />
                </div>
                <div>
                ...
                <div>
                  ...
                  <input className="input" value={form.whatsapp} onChange={e => setForm({...form, whatsapp: e.target.value})} placeholder="(11) 99999-9999" />
                </div>
                <div className="col-span-2">
                  <label className="label">Endereço</label>
                  <input className="input" value={form.endereco} onChange={e => setForm({...form, endereco: e.target.value})} placeholder="Rua, número, complemento" />
                </div>
                <div>
                ...
                <div className="col-span-2">
                  ...
                  <textarea className="input" rows={3} value={form.observacoes} onChange={e => setForm({...form, observacoes: e.target.value})} placeholder="Anotações internas..." />
                </div>
              </div>
        ...
        <div className="card p-0 overflow-hidden">
          {loading ? (
            <div className="text-center py-12 text-gray-400 text-sm">Carregando clientes...</div>
          ) : filtrados.length === 0 ? (
              ...
              <tbody>
                {filtrados.map((c) => (
                  <tr key={c.id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3"><button className="btn text-xs py-1 px-2.5">Ver ficha</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
